...
def test_get_by_id_returns_expense():
    ...
    """
    repo = InMemoryExpenseRepository()
    expense = create_expense(id=1, title="Object", amount=15)
    repo.save(expense)
    expense_by_id = repo.get_by_id(1)
    assert expense.id == expense_by_id.id
    assert expense.title == expense_by_id.title
    assert expense.amount == expense_by_id.amount
...
def test_get_by_id_returns_none_if_not_found():
    ...
    expense = create_expense(id=1, title="Object", amount=15)
    repo.save(expense)
    expense_by_id = repo.get_by_id(2)
    assert expense_by_id is None
